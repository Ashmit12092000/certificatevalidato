...
def report():
    ...
    conn = get_db_connection()
    customers = conn.execute("SELECT id, name, code FROM customers ORDER BY name ASC").fetchall()
    conn.close()

    return render_template("demo_report.html", customers=customers)
@app.route('/report/<int:customer_id>')
def customer_report_detail(customer_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    certificates = conn.execute("""
        SELECT * FROM certificates WHERE customer_id = ? ORDER BY cert_type
    """, (customer_id,)).fetchall()

    conn.close()

    return render_template("report_detail.html", customer=customer, certificates=certificates)
@app.route('/report/customer/<int:customer_id>/download')
def download_customer_report(customer_id):
    ...
    certificates = conn.execute("""
        SELECT cert_type, status, activation_date, expiration_date, verified
    ...
    writer.writerow(["Certificate Type", "Status", "Activation Date", "Expiration Date", "Verified"])

    for cert in certificates:
        writer.writerow([cert["cert_type"], cert["status"], cert["activation_date"],
                         cert["expiration_date"], "Yes" if cert["verified"] else "No"])

    output.seek(0)
    return Response(output, mimetype="text/csv",
                    headers={"Content-Disposition": f"attachment;filename={customer['name']}_certificates.csv"})
